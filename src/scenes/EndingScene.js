...
export class EndingScene extends Phaser.Scene {
   ...
   */
  _getEndingBGMType(endingKey) {
    const legendSet = ['legend', 'warrior', 'comeback'];
    const tycoonSet = ['tycoon', 'ai_visionary'];
    const dangerSet = ['scapegoat', 'supply_chain'];
    const peaceSet = ['balance', 'peace', 'default'];

    let color, alpha, pattern;
    if (legendSet.includes(endingKey)) {
      color = COLORS.accent;
      alpha = 0.2;
      pattern = 'radial';
    } else if (tycoonSet.includes(endingKey)) {
      color = 0x40c040;
      alpha = 0.15;
      pattern = 'rain';
    } else if (dangerSet.includes(endingKey)) {
      color = COLORS.danger;
      alpha = 0.2;
      pattern = 'rain';
    } else {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const dotAlpha = alpha + (Math.random() - 0.5) * 0.1;
      const dot = this.add.graphics();
      dot.fillStyle(color, Math.max(0.05, dotAlpha));
      ...
      if (pattern === 'radial') {
        ...
        const maxDist = Math.max(GAME_WIDTH, GAME_HEIGHT) * 0.7;
        ...
        this.tweens.add({
          targets: dot,
          x: cx + Math.cos(angle) * maxDist,
          y: cy + Math.sin(angle) * maxDist,
          alpha: { from: Math.max(0.05, dotAlpha), to: 0 },
          duration: dur,
          ...
          onRepeat: () => {
            dot.x = cx;
            dot.y = cy;
            dot.alpha = Math.max(0.05, dotAlpha);
          }
        });
        ...
        const px = Phaser.Math.Between(0, GAME_WIDTH);
        const startY = Phaser.Math.Between(-GAME_HEIGHT, 0);
        const dur = Phaser.Math.Between(10000, 20000);
        dot.setPosition(px, startY);
