// R77 验证：F1 已读标记时机（P0）+ F2 AUTO 自动推进 + F3 reduce 模式跳过相机震动 + F4 ESC 关面板不重开菜单
// 用法: node scripts/_verify_r77.cjs [baseUrl]   默认 http://localhost:4173/luohammer-pixel-game/
const { chromium } = require('playwright');
const path = require('path');
const BASE = process.argv[2] || 'http://localhost:4173/luohammer-pixel-game/';
const OUT = path.resolve(__dirname, '..', '..', 'test-screenshots', 'r77-verify');
...
async function openFresh(browser, node, stage, opts = {}) {
  ...
  }, { s: seed(node, stage), seen: opts.seenNodes || null });
...
(async () => {
  ...
  // ================= F1a【P0】：首读节点零交互 → 无「已读」角标、不自动播放 =================
  {
    ...
    await page.waitForTimeout(2000); // 等 makeChoice → transition → loadNode
    const seen = await page.evaluate(() => JSON.parse(localStorage.getItem('luohammer_seen_nodes') || '{}'));
  ...
  }
  {
    ...
    assert('F3-reduce-noshake', 'reduce 模式相机震动被跳过', !r.running, `shakeRunning=${r.running}`);
  ...
  console.log(`\n== R77 VERIFY 小计: PASS ${p} / FAIL ${results.length - p} ==`);
